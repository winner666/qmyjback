...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        ...
        str = "<div><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div> <div><input type='button' class='am-btn am-btn-default am-btn-sm' value='取消' onclick='delRow(this)'></div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        document.getElementById("picUrl").value = link;
    ...
    var showImg = $('<img name="imgView" src="assets/js/qianniuJs/img/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', 'assets/js/qianniuJs/img/default.png');
